...
public class ClientInputMain {
		...
		while (!done) {
			...
			try {
				...
				case "5":
					...
					break;
				case "6":
					eventBus.sendEvent(new Event(EventId.RegisterReservations, makeReservationInfo()));
					printLogSend(EventId.RegisterReservations);
					break;
				case "7":
	...
	private static String makeReservationInfo() throws IOException {
		return setStudentId()+" "+setCourseId();
	...
	private static String setStudentId() throws IOException {
		...
		while(out.length()!=8) {
			System.out.println("\nEnter student ID and press return (Ex. 20131234)>> ");
			out = new BufferedReader(new InputStreamReader(System.in)).readLine().trim();
		}
		return out;
	}
	//@SuppressWarnings("unused")
	...
	private static void writeMenu() {
		...
		System.out.println("6. Register a new Reservations");
